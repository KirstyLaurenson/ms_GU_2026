...
#### guillemot recruitment at age 6
...
df <- read.csv("standardised_covariates_for_models_1995-2020.csv")

####  estimate impact of single covariate on age 6 recruitment  ####

# there are no immature guillemots aged 6 in the first 5 years of the model as
# all guillemots enter the model as chicks - so remove first 5 years of covariates
...
myCode <- nimbleCode ({
  ...
  for (t in 1:(K-1)) { # loop over time
    ...
    logit(alpha.6[t]) <- beta[1] + beta[2] * cov1[t] + eps[t]    # derived parameter 
...
# run nimble 
system.time({
  mcmc.alpha6.wpc1 <- nimbleMCMC(code = myCode,
...
save(mcmc.alpha6.wpc1, file = "alpha6_wpc1.RData")

####  estimate impact of two covariates on age 6 recruitment  ####

# remove first 5 years of covariates
...
myCode <- nimbleCode ({
  ...
  for (t in 1:(K-1)) { # loop over time
    ...
    logit(alpha.6[t]) <- beta[1] + beta[2] * cov1[t] + beta[3] * cov2[t] + eps[t]    # derived parameter 
...
# run nimble 
system.time({
  mcmc.alpha6.wpc1.wsst <- nimbleMCMC(code = myCode,
                                 constants = my.constants,
                                 data = my.data,
                                 inits = initial.values,
                                 monitors = parameters.to.save,
...
save(mcmc.alpha6.wpc1.wsst, file = "alpha6_wpc1_wsst.RData")

####  estimate impact of two covariates and their interaction on age 6 recruitment  ####
...
myCode <- nimbleCode ({
  ...
  beta[2] ~ dnorm(0, sd = 1.5)                    # prior slope
  beta[3] ~ dnorm(0, sd = 1.5)                    # prior slope
  beta[4] ~ dnorm(0, sd = 1.5)                    # prior slope interaction
  sdeps ~ dunif(0, 10)                            # sd of variance 
  ...
  for (t in 1:(K-1)) { # loop over time
    ...
    logit(alpha.6[t]) <- beta[1] + beta[2] * cov1[t] + beta[3] * cov2[t] + beta[4] * (cov1[t] * cov2[t]) + eps[t]    # derived parameter 
...
# run nimble 
system.time({
  mcmc.alpha6.wpc1.wsst.int <- nimbleMCMC(code = myCode,
                                      constants = my.constants,
                                      data = my.data,
                                      inits = initial.values,
                                      monitors = parameters.to.save,
...
save(mcmc.alpha6.wpc1.wsst.int, file = "alpha6_wpc1_wsst_int.RData")