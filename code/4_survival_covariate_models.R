#### Code to run multistate CMR models estimating covariate impacts on juvenile 
#### and immature guillemot survival
...
my.constants <- list(N = nrow(y),   # individuals
                     ...
                     first = first, 
                     cov1 = cov$son_pc1) # single covariate model - change covariate as appropriate
...
### nimble model:
# can be used to estimate covariate impacts on juvenile survival (ages 1&2) or 
# immature survival age 4 - add or remove comments accordingly
myCode <- nimbleCode ({
  # ------------------------------------------------------
  # Parameters:
  # phi.1: first year survival probability
  ...
  phi.3 ~ dunif(0,1)                              # constant age 3 survival
  ...
  # phi.1[t] ~ dunif(0,1)                         # need this prior when testing covariate impacts on phi.4
    ...
    logit(phi.1[t]) <- beta[1] + beta[2] * cov1[t] + eps[t]    # derived parameter - age1 survival
  # logit(phi.4[t]) <- beta[1] + beta[2] * cov1[t] + eps[t]    # derived parameter - age4 survival
...
initial.values <- function() list(beta = rnorm(2,0,1),
                                  sdeps = runif(1,0,3),
                                # phi.1 = runif(my.constants$K-1,0,1),   # need an initial value for phi.1 when running phi.4 covariate models
                                  phi.3 = runif(1,0,1),
                                  phi.4 = runif(my.constants$K-1,0,1),   # remove phi.4 initial values when running phi.4 covariate models
...
# run nimble 
system.time({
  mcmc.phijuv.apc1 <- nimbleMCMC(code = myCode,
...
# change model name accordingly for covariates
save(mcmc.phijuv.apc1, file = "phijuv_apc1.RData")

####  estimate impact of two covariates on juvenile survival  ####
...
myCode <- nimbleCode ({
  ...
  beta[1] ~ dnorm(0, sd = 1.5)                    # prior intercept
  beta[2] ~ dnorm(0, sd = 1.5)                    # prior slope
  beta[3] ~ dnorm(0, sd = 1.5)                    # prior slope
  sdeps ~ dunif(0,10)
  phi.3 ~ dunif(0,1)                              # constant age 3 survival
  ...
  for (t in 1:(K-1)) { # loop over time
    ...
    logit(phi.1[t]) <- beta[1] + beta[2] * cov1[t] + beta[3] * cov2[t] + eps[t]    # derived parameter - age1
...
# initial values
initial.values <- function() list(beta = rnorm(3,0,1),
                                  sdeps = runif(1,0,3),
                                  phi.3 = runif(1,0,1),
...
# run nimble 
system.time({
  mcmc.phijuv.apc1.ssst <- nimbleMCMC(code = myCode,
                                 constants = my.constants,
                                 data = my.data,
                                 inits = initial.values,
                                 monitors = parameters.to.save,
...
save(mcmc.phijuv.apc1.ssst, file = "phijuv_apc1_ssst.RData")

####  estimate impact of two covariates and their interaction on juvenile survival  ####
...
myCode <- nimbleCode ({
  ...
  beta[2] ~ dnorm(0, sd = 1.5)                    # prior slope
  beta[3] ~ dnorm(0, sd = 1.5)                    # prior slope
  beta[4] ~ dnorm(0, sd = 1.5)                    # prior interaction slope
  sdeps ~ dunif(0,10)
  phi.3 ~ dunif(0,1)                              # constant age 3 survival
  ...
  for (t in 1:(K-1)) { # loop over time
    ...
    logit(phi.1[t]) <- beta[1] + beta[2] * cov1[t] + beta[3] * cov2[t] + beta[4] * (cov1[t] * cov2[t]) + eps[t]    # derived parameter - age1
...
# initial values
initial.values <- function() list(beta = rnorm(4,0,1),
                                  sdeps = runif(1,0,3),
                                  phi.3 = runif(1,0,1),
                                  phi.4 = runif(my.constants$K-1,0,1),
                                  phi.5 = runif(my.constants$K-1,0,1),
...
# run nimble 
system.time({
  mcmc.phijuv.apc1.ssst.int <- nimbleMCMC(code = myCode,
                                      constants = my.constants,
                                      data = my.data,
                                      inits = initial.values,
                                      monitors = parameters.to.save,
...
# change model name accordingly 
save(mcmc.phijuv.apc1.ssst.int, file = "phijuv_apc1_ssst_int.RData")
